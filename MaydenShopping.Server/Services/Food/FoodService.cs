...
namespace MaydenShopping.Server.Services.Food
{
    public class FoodService : ServiceBase, IFoodService
    {
        ...
        private readonly IRepository<FoodItem> repository;
        private readonly IRepository<ShoppingListItem> shoppingListItemRepository;
        ...
        public async Task<IActionResult> DeleteItem(int id, CancellationToken token = default)
        {
            ...
            List<ShoppingListItem> shoppingListItems = await shoppingListItemRepository.Get().Where(item => item.FoodItemId == id).ToListAsync(token);
        ...
        public async Task<IActionResult> GetItems(RequestList request, CancellationToken token = default)
        {
            ...
            int skip = (request.PageNumber - 1) * request.Showing;
            int count = await repository.Get().CountAsync(token);
            List<FoodResponse> results = await repository.Get().Skip(skip).Take(request.Showing).Select(foodItem => ResponseMappings.MapToResponse(foodItem)).ToListAsync(token);
        ...
        public async Task<IActionResult> InsertItem(FoodRequest request, CancellationToken token = default)
        {
            ...
            bool isAlreadyAdded = await repository.Get().AnyAsync(foodItem => foodItem.Name.ToLower() == request.Name.ToLower().Trim() || foodItem.Barcode == request.Barcode.ToLower(), token);
            if (isAlreadyAdded)
            {
